...
const RESPONSE_WEIGHTS = {
  ...
  namedInSalutation:      3,   // the greeting names the signed-in user — also a hard gate in maybeAddToWatchlist
...
// The "answered?" check. Asks Graph for the most recent message in the thread:
// if it came from our side, the thread is answered. A reply from ANY Setty
// account — including a colleague's — counts, so a teammate handling it clears
// the item for everyone. Returns { awaiting, since } where `since` is the
// latest message's timestamp (ms) — when awaiting, that's the unanswered client
// message, i.e. when our response clock starts. Falls back to
// { awaiting: true, since: null } when Graph can't tell, so a flagged email
...
async function isThreadAwaitingReply(conversationId) {
  ...
  try {
    ...
    // Sent Items — which made every replied-to thread look permanently
    // unanswered. Query Sent Items explicitly and merge; its failure is
    // non-fatal (worst case we're back to the old inbox-only view).
    const [all, sent] = await Promise.all([
      graphFetch("GET", "/me/messages" + qs),
      graphFetch("GET", "/me/mailFolders/sentitems/messages" + qs).catch(() => null),
    ]);
    const sentIds = new Set((sent?.value || []).map(m => m.id));
    const byId = new Map();
    [...(all?.value || []), ...(sent?.value || [])].forEach(m => byId.set(m.id, m));
    ...
    const latest = msgs[0];
    const latestSender = (latest.from?.emailAddress?.address || "").toLowerCase();
    const myAddr = (msalAccount?.username || "").toLowerCase();
    // "From us" = it sits in Sent Items (definitionally ours, whatever address
    // it went out under), OR the sender is any Setty account, OR it matches the
    // signed-in address — the last covers replies sent from a personal alias.
    const fromUs = sentIds.has(latest.id) ||
      isSettyInternalEmail(latestSender) ||
      (!!myAddr && latestSender === myAddr);
    return {
      awaiting: !fromUs,
      since: when(latest) || null,
...
async function maybeAddToWatchlist(myGen) {
  try {
    ...
    const canCheckName = myFirstName.length >= 2;
    const namedInSalutation = canCheckName &&
      new RegExp("\\b" + myFirstName + "\\b", "i").test(text.slice(0, 80));
